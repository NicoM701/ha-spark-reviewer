...
3. The app does not expose a shell or setup website. Use the authenticated Studio Code terminal to run trusted Codex and GitHub CLI login tools with their home directories pointing to this app's mapped private configuration folder. Discover the actual folder rather than guessing it. Authenticate with `codex login --device-auth` and `gh auth login --web`. Their corresponding app-container paths are `/config/codex` and `/config/gh`. This terminal path and CLI availability must be verified after HA sign-in. Use browser approval, never paste credentials into chat. Only this app's credentials should live in that folder.
   ...
   On the verified Green installation, the Studio Code mapping is `/addon_configs/a49a9170_spark_review`. Export `CODEX_HOME` to its `codex` subfolder and `GH_CONFIG_DIR` to its `gh` subfolder before logging in. The installed Studio Code image did not include these CLIs on PATH; official ARM64 Codex 0.153.4 and GitHub CLI 2.100.0 binaries worked from a temporary setup directory. Keep that directory separate from Home Assistant configuration. Confirm `codex login status` reports ChatGPT authentication and `gh api user --jq .login` reports the intended PR author. Both credential files should be readable only by their owner, with mode `600`.
